...
This repo contains the source for components that resides outside of the dotnet
...
is responsible building the submodule with the appropriate configuration for
...
must be applied via [patches](patches).
...
1. [Build](#how-to-build) locally
    ...
    1. Resolve build errors. Source changes must be applied via [patches](patches).
...
When creating/updating patches, it is desireable to backport the changes whenever feasible as this reduces
